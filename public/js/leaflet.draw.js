var xPos = 0, yPos = 0;
var vertextCnt = 0;
var displayCurLength = 0;
var polygonArea = 0, offset = 0;

var firstLength = 0;
var pixelInchRatio = 0.0;
var feet_inch_str = "";
var line_midLatLng;
...
!function(t, e, i) {
    ...
    L.Draw.Polyline = L.Draw.Feature.extend({
        ...
        addVertex: function(t) {
        	vertextCnt = this._markers.length;
            ...
            if (vertextCnt == 1 && pixelInchRatio == 0) {
                var firstLatLng = this._poly.getLatLngs()[0];
                var lastLatLng = this._poly.getLatLngs()[1];

                var firstPt = new L.Point(firstLatLng.lat, firstLatLng.lng);
                var lastPt = new L.Point(lastLatLng.lat, lastLatLng.lng);

                firstLength = firstPt.distanceTo(lastPt);
            }
        },
        ...
        calcPolygonArea: function(slop_id = 0) {
            var total = 0;

            var vertices = this._poly.getLatLngs();
            ...
            var area = Math.round((Math.abs(total) * pixelInchRatio * pixelInchRatio)/(12 * 12));
            area = (Math.sqrt(12*12 + slop_id*slop_id)/12) * area;

            return area;
        },
        _finishShape: function() {
            polygonArea = this.calcPolygonArea();
        ...
        _updateTooltip: function(t) {
            if (this._markers.length > 0) {
                ...
                var curPt = new L.Point(this._currentLatLng.lat, this._currentLatLng.lng);
                var lastPt = new L.Point(lastLatLng.lat, lastLatLng.lng);

                var dist = lastPt.distanceTo(curPt);
                if (pixelInchRatio == 0) {
                    return;
                } if (zoomSl == -1) {
                    zoomSz = 0;
                ...
                line_midLatLng = L.latLng(lastPt.x + (curPt.x - lastPt.x)/2, lastPt.y + (curPt.y - lastPt.y)/2 - (zoomSz / zoomSl));

                inch_dist = pixelInchRatio * dist;
                inch_dist = Math.round(inch_dist);

                feet = Math.floor(inch_dist/12);
                inch = inch_dist%12;

                feet_inch_str = "";
                if (feet > 0) {
                    feet_inch_str += feet.toString() + "ft";
                }

                if (inch > 0) {
                    feet_inch_str += " " + inch.toString() + "in";
                }
                
                e = {
                    text: "",
                    subtext: feet_inch_str
                };
    ...
    L.Edit.PolyVerticesEdit = L.Handler.extend({
        ...
        _onMarkerDrag: function(t) {
            ...
            for (var i=0; i<s.length; i++) {
                ...
                secondPt = s[i+1];
                if (i == s.length-1) {
                    firstPt = s[s.length-1];
                    secondPt = s[0];
                }

                var pt1 = new L.Point(firstPt.lat, firstPt.lng);
                var pt2 = new L.Point(secondPt.lat, secondPt.lng);
        ...
        _GetLineLenAndPt: function(firstPt, secondPt) {
            ...
            line_midLatLng = L.latLng(secondPt.x + (firstPt.x - secondPt.x)/2, secondPt.y + (firstPt.y - secondPt.y)/2 - (zoomSz / zoomSl));
            ...
            return {line_midLatLng, feet_inch_str};
        },
